...
const registerController = async (req, res) => {
    try {
        ...
        const templatePath = path.join(__dirname, "../views/email/email-verify.ejs");
...
const verifyEmailController = async (req, res) => {
    ...
    try {
        ...
        if (!token || typeof token !== "string") {
            return res.redirect(`${loginRedirectUrl}?status=error&message=invalid_token`);
        }
        ...
        if (!user) {
            return res.redirect(`${loginRedirectUrl}?status=error&message=token_not_found`);
        }
        // 4. User is already verified
        if (user.email_verified_at) {
            return res.redirect(`${loginRedirectUrl}?status=info&message=already_verified`);
        }
        // 5. Token expiration boundary check
        if (user.email_token_expires && new Date() > user.email_token_expires) {
            return res.redirect(`${loginRedirectUrl}?status=error&message=link_expired`);
        }
        ...
        return res.redirect(`${loginRedirectUrl}?status=success&message=email_verified`);
    }
    catch (error) {
        console.error("Email verification pipeline error:", error);
        // Safe fallback to frontend login in case of system/database crash
        return res.redirect(`${loginRedirectUrl}?status=error&message=server_error`);
    }
};
const loginController = async (req, res) => {
    try {
        ...
        const token = jwt.sign({ userId: user.id, email: user.email }, process.env.JWT_SECRET || "smash_arena_fallback_secret_key", { expiresIn: "1d" });
...
export { registerController, verifyEmailController, loginController, logoutController, checkSessionController, };
